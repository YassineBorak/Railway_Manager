...
**Exemple avec `prompt-sync` :** Il faut installer le package avec : `npm install prompt-sync`
...
Après chaque opération, le programme doit revenir au menu principal. Le programme s'arrête uniquement lorsque l'utilisateur choisit :
...
**Chiffre d'affaires total** Calculer la somme des prix des tickets.

Chiffre d'affaires total : 1 250 DH

**Trajet le plus vendu** Compter le nombre de tickets correspondant à chaque `tripId`.
